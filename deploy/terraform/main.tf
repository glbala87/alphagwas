# AlphaGWAS AWS Infrastructure
# Terraform configuration for deploying AlphaGWAS to AWS
...
# ECR Repository
resource "aws_ecr_repository" "alphagwas" {
...
resource "aws_ecr_lifecycle_policy" "alphagwas" {
  repository = aws_ecr_repository.alphagwas.name
...
resource "aws_cloudwatch_log_group" "alphagwas" {
...
resource "aws_ecs_task_definition" "api" {
  ...
  container_definitions = jsonencode([
    {
      name  = "api"
      image = "${aws_ecr_repository.alphagwas.repository_url}:latest"
      ...
      logConfiguration = {
        logDriver = "awslogs"
        options = {
          "awslogs-group"         = aws_cloudwatch_log_group.alphagwas.name
  ...
  container_definitions = jsonencode([
    {
      ...
      command = ["python", "run_pipeline.py", "--daemon"]
